		...
		union(){
			translate([0,-idler_y_offset+2.5,0])
				rotate([90,0,0]){
					...
					translate([xend_body_x_offset + 20,idler_elevation,0])
						cylinder(r=m8_washer_dia/2, h=5,center=true);
					translate([0,idler_elevation-xend_body_length/2 + 10])
						linear_extrude(height=5,center=true) polygon(points=[[-xend_body_length/2+xend_body_x_offset,m8_washer_dia/2/sin(45)],
															...
															[xend_body_length/2+xend_body_x_offset,0],
															[xend_body_length/2+xend_body_x_offset,xend_body_length * 0.6],
															[xend_body_length/2+xend_body_x_offset-m8_washer_dia/2/sin(45),xend_body_length * 0.6]]);
				}

			// idler mount support top
			difference(){
				union() {
				translate([xend_body_length/2+xend_body_x_offset-(m8_washer_dia * 0.2 / sin(45))/2,-idler_y_offset/2 - 5,xend_body_height/2+xend_body_length - 29.5])
					cube([m8_washer_dia * 0.2 / sin(45), idler_y_offset - 10, m8_washer_dia * 0.2 / sin(45)],center=true);
				// angled support
				translate([xend_body_length/2+xend_body_x_offset-(m8_washer_dia * 0.2 / sin(45))/2,-idler_y_offset/2 - 5 + 16.5,xend_body_height/2+xend_body_length - 50])
				rotate([292.5, 0, 0])
					cube([m8_washer_dia * 0.2 / sin(45),idler_y_offset + 10,m8_washer_dia * 0.35 / sin(45)],center=true);
				}	// close union
				translate([xend_body_x_offset*2,0,0])
					mirror([1,0,0]){
						translate([-(body_width+1)/2-zrod_leadscrew_dist/2,0,body_length/2])
...
//				cube([m8_washer_dia/2/sin(45),idler_y_offset,xend_body_height]);
		...
		// idler bolt hole
		translate([xend_body_x_offset + 20,-idler_y_offset+2.5,idler_elevation])
		rotate([0,0,90])
			teardrop(m8_screw_dia/2+clearance,7);

		// bearing support access hole
		translate([xend_body_x_offset + 15, -idler_y_offset + 2.5, xend_body_height + 4])
		rotate([0,0,90])
			teardrop(m3_screw_dia + clearance, 7);
	}
/*
	mirror([0,1,0])
	difference(){
