...
yidler(h, w, m1, m2, st);
translate([-5,0,0]) mirror([1,0,0]) yidler(h, w, m1, m2, st);

module yidler(yih, yiw, yim, bim, yst)
{
	translate([20,2,0]) rotate([90,0,180]) wall(yih,yim,yst);
	translate([0,h+2-o,0]) base(yih, yiw, bim);
}
...
module base(bh,bw,bm)
{
	difference()
	{
		...
		translate([-o, 17, bh]) rotate([0,0,0]) cube([25, 15, 25]);
		translate([20, -o, -o]) rotate([0,0,0]) cube([10, 20, 21]);
		translate([-10, 1.5, -o]) rotate([0,0,0]) cube([10, 25, 21]);
		translate([-10, 21.2, -o]) rotate([0,0,-45]) cube([10, 20, bh+2*o]);
		translate([27, 10, -o]) rotate([0,0,45]) cube([10, 20, bh+2*o]);
