...
{
	translate([20, (th/cos(30)) - o, 0]) rotate([0,0,150]) walls(th,st,hs);
	base(th,hs);
}
...
module walls(wth,wst,whs)
{
	...
	{
		cube([ln,wth,20+wth]);
		translate([10, -wth / 2, 16]) rotate([-90,0,0]) cylinder(r=whs/2,h = wth * 2);
	}

	translate([ln, wth + o, 0]) rotate([0,0,210]) difference()
	{
	...
	difference()
		{
			...
			translate([11,15+bth,-o]) cylinder(r=bhs/2,h = 2 * bth);
			translate([11,35+bth,-o]) cylinder(r=bhs/2,h = 2* bth);
		}
}
